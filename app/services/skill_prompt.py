...
10. "받침"이라는 단어는 어떤 목표 언어로도 번역하지 않는다. 문법 구조 설명 문장 안에
    ...
    만들어내지 않는다).
10b. 문법 구조 다이어그램의 품사 줄임말 라벨("동", "명", "형", "부", "관", "감", "조" —
    도형 텍스트가 이 한 글자뿐인 경우)도 "받침"과 동일하게 어떤 목표 언어로도 번역하지
    않고 원문 그대로 유지한다(action="skip"). "동사"라고 풀어서 번역하거나 목표 언어
    단어(예: "Глагол")로 바꾸지 않는다 — 여러 슬라이드에 반복되는 라벨인데 배치마다
    다르게 번역하면 같은 문서 안에서 통일이 깨진다.
